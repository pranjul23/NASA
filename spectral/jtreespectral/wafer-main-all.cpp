...
int main (int argc, char *argv[])
{
 ...
 /* int num_hidden_states = 2;
  ...
  int EM_FLAG = 1;
  int num_hidden_nodes = 152;
  int num_observed_nodes = 152;
...
//  int num_training_examples = 2675;
  int num_testing_examples = 3582;
  int num_columns = 153;
  int num_obs_vars = 152;

  string train_file_name = "wafer_train-" + to_string(trial) + ".txt";
  string test_file_name = "wafer_test-" + to_string(trial) + ".txt";
...
/* int jtree_nodes = num_observed_nodes - skip;
 ...
 /* vector<int> jtemplate_vec(jtree_nodes, 0);
  ...
  {
	//  if ((i / 10.0) > curr_template)
	//	  curr_template++;
	jtemplate_vec[i] = curr_template++;
  }
//  curr_template++;
  jtemplate_vec[jtree_nodes/2 - 2] = curr_template++;
  jtemplate_vec[jtree_nodes/2 - 1] = curr_template++;
 ...
 /* int EM_jtree_nodes = num_observed_nodes;
  ...
  jtree0.LearnTemplateSpectralParameters();
  ...
  jtree1.LearnTemplateSpectralParameters();
 ...
 /*  double EM_time1 = 0;
  ...
  {
		clock_t i_EM_init = clock();
		JTree* i_EMtree = new JTree(bayesNet1, EM_tree_matrix, EM_nodes_to_vars, train_samples0, EM_thresh, 0);
		double like = i_EMtree->LearnEMParameters(10, 0);
		clock_t i_EM_final = clock() - i_EM_init;
  ...
  {
		clock_t i_EM_init = clock();
		JTree* i_EMtree = new JTree(bayesNet1, EM_tree_matrix, EM_nodes_to_vars, train_samples0, EM_thresh, 1);
		double like = i_EMtree->LearnOnlineEMParameters(online_rates[i], 0, 1000000);
		clock_t i_EM_final = clock() - i_EM_init;
...
/*	double EM_val0 = EMtree0->ComputeEmpiricalMarginals(marginals, EM_evidence_vars, EM_evidence_vals);
	...
	if (EM_online_val0 > EM_online_val1 && EM_online_val0 > EM_val2)
	{
		online_EM_map = 0;
	}
...
/*	cout << EM_val0;
  ...
  cout << "yeeee\n";
  spectral_error = spectral_error / 1000;
  ...
  string outfile_name = "wafer-" + to_string(num_hidden_states) + "-"
...
//  string outfile_name = "fun.txt";
  ...
  myfile << ("EMt" + to_string(online_EM_error) + "\t" + to_string(EM_time) + "\n");
