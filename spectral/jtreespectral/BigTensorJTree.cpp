...
#include "C:\\Research\\spectral-latent-tree\\repository\\code-c++\\Eigen\\Dense"
#include "C:\\Research\\spectral-latent-tree\\repository\\code-c++\\Eigen\\SVD"
...
// default constructor
TensorJTree::TensorJTree(BayesianNetwork* bNet, Matrix& tree_matrix, vector<vector<int>*>& nodes_to_vars, int max_obs_per_mode, vector<int>& template_vec)
{
	elimination_order = NULL;
	this->bNet = bNet;
	...
	{
		node_list->push_back(new TensorJTNode(this, i, max_obs_per_mode));
	}
...
void TensorJTree::FindObservationPartitions(int clique_index)
{
	...
	//	VectorPlus::SetDiff(other_nodes, *all_obs_vars, union_obs_vars);
	...
	for (iter = unique_child_seps.begin(); iter != unique_child_seps.end(); ++iter)
	{
		...
		node_list->at(clique_index)->AddObsPartition(sep, extra_obs, sep_nodes.size());
	}
...
vector<int>& TensorJTree::GetDescendants(int clique_index, vector<int>& mode_group, int index) { return node_list->a