...
/*  int jtree_nodes = 2 * (num_observed_nodes - skip);
  ...
  for (int i = jtree_nodes / 2; i < jtree_nodes; ++i)
  {
	  ...
	  if (i == jtree_nodes / 2)
	  {
		  ...
		  nodes_to_vars[i]->push_back(hidden_nodes[i - jtree_nodes / 2 + skip + 2]);
 ...
 double EM_time = 0;
  ...
  for (int i = 0; i < num_EM_trials; ++i)
  {
	...
	int win_count = 0;
	  ...
	  string outfileval_name = "third-nh-hmm-vals-" + to_string(num_hidden_nodes) + "-" + to_string(num_hidden_states) + "-" 
