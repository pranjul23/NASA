...
//  int num_training_examples = 2675;
  ...
  vector<int> one_dims  = VectorPlus::CreatePair(one_count + two_count, num_nodes);
...
/*	cout << EM_val0;
	...
	if ((spectral_map == 0 && test_labels[n] != 0) || (spectral_map != 0 && test_labels[n] == 0))
	{
		spectral_error++;
	}
	if ((EM_map == 0 && test_labels[n] != 0) || (EM_map != 0 && test_labels[n] == 0))
	{
		EM_error++;
	}
	if ((online_EM_map == 0 && test_labels[n] != 0) || (online_EM_map != 0 && test_labels[n] == 0))
	{
		online_EM_error++;
	}
  }
  ...
  string outfile_name = "splicetwo2-" + to_string(num_hidden_states) + "-"
