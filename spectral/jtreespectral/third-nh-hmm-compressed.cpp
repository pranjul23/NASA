...
/*  int jtree_nodes = num_observed_nodes - skip;
  ...
  for (int i = jtree_nodes / 2; i < jtree_nodes; ++i)
  {
    ...
    int compressed_jtree_nodes = 2 * (num_observed_nodes - skip) - 3;
  int compressed_first_nodes = (num_observed_nodes - skip) - 3;
  ...
  for (int i = compressed_first_nodes; i < compressed_jtree_nodes - 3; ++i)
  {
	  int parent = i - compressed_first_nodes;
	  compressed_tree_matrix.Set(i, i - compressed_first_nodes, 1);
  }
  compressed_tree_matrix.Set(compressed_jtree_nodes - 3, compressed_first_nodes - 1, 1);
  ...
  for (int i = 0; i < compressed_first_nodes;  ++i)
  {
	  ...
	  compressed_nodes_to_vars[i]->push_back(hidden_nodes[i+3 + skip]);
  }
...
/* vector<int> jtemplate_vec(jtree_nodes, 0);
 ...
 /* int EM_jtree_nodes = num_observed_nodes;
  ...
  TensorJTree jtree(bayesNet, compressed_tree_matrix, compressed_nodes_to_vars, stack_count, jtemplate_vec, max_runs);
  ...
  for (int i = 0; i < num_EM_trials; ++i)
  {
		...
		JTree* i_EMtree = new JTree(bayesNet, compressed_tree_matrix, compressed_nodes_to_vars, train_samples, EM_thresh, 1);
