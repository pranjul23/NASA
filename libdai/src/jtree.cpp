...
Factor JTree::belief( const VarSet &vs ) const {
    ...
    for( beta = Qb.begin(); beta != Qb.end(); beta++ )
        if( beta->vars() >> vs ) break;
    if( beta != Qb.end() ) {

    	if( props.verbose >= 3 ) {
    	     cerr << " VarSet: " << vs << "\n" << "is found among separators\n";
    	}
...
void JTree::runHUGIN() {
	...
	//when we assign OR to Qa here, we incorporate evidence from clamped variables
    for( size_t alpha = 0; alpha < nrORs(); alpha++ )
        Qa[alpha] = OR(alpha);
...
//      IR(i) = seperator OR(RTree[i].first) && OR(RTree[i].second)
        ...
        if( props.verbose >= 3 ) {
        	cerr << Qa[RTree[i].second].vars() << " --> " << Qb[i].vars() << " --> " << Qa[RTree[i].first].vars( )<<"\n";
            cerr << "--> Old separator: " << Qb[i] << endl;
            cerr << "--> Old potential: " << Qa[RTree[i].first] << endl<<endl;
        }
        ...
        //cerr << "--> new_Qb.norm(): " << new_Qb.normalize() << endl;

        //here we calculate normalization constant as well as perform normalization itself
        ...
        if( props.verbose >= 3 ) {
        	cerr << "\n--> New separator: " << Qb[i] << endl;
        	cerr << "--> New potential: " << Qa[RTree[i].first] << endl;
        }
...
//      IR(i) = seperator OR(RTree[i].first) && OR(RTree[i].second)
        ...
        if( props.verbose >= 3 ) {
        	cerr << Qa[RTree[i].second].vars() << " <-- " << Qb[i].vars() << " <-- " << Qa[RTree[i].first].vars( )<<"\n";
           	cerr << "<-- Old separator: " << Qb[i] << endl;
           	cerr << "<-- Old potential: " << Qa[RTree[i].second] << endl << endl;
        }
        ...
        if( props.verbose >= 3 ) {
        	cerr << "\n<-- New separator: " << Qb[i] << endl;
           	cerr << "<-- New potential: " << Qa[RTree[i].second] << endl;
        }
    ...
    for( size_t alpha = 0; alpha < nrORs(); alpha++ ){
    	cout << Qa[alpha].vars() << ",  norm: " << Qa[alpha].normalize() << "\n";
    	cout << Qa[alpha] << "\n";
    }
    cout << "\n";
    */
...
/*
vector<Factor> JTree::calcDistrib(const vector<VarSet> &vs, const size_t& margLabel){
	...
	for(VarSet::const_iterator v = tmp.begin(); v != tmp.end(); v++){
		...
		//create this varset to be used later in marginalization
		//we will marginalize all variables except Y
		if(i != margLabel){
			Y |= Var(i, v->states());
		}
	...
	//numerator of output Factor
	Factor numerator(X, 0.0), term(X, 0.0);
	...
	cout << "Denominator: " << numerator.marginal(Y, false) << "\n";


	res.push_back(numerator);
	res.push_back(numerator.marginal(Y, false));
...
Factor JTree::calcDistrib(const vector<VarSet> &vs){
...
void JTree::normDistrib(Factor &f, const size_t& margLabel){
	...
	for(VarSet::const_iterator v = f.vars().begin(); v != f.vars().end(); v++){

		//create this varset to be used later in marginalization
		//we will marginalize all variables except variables in X
