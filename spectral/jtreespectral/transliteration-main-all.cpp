...
/*  int num_hidden_states = 2;
  ...
  int num_observed_nodes = 20;
  int num_observed_states = 27;
  // node names
...
//  int num_training_examples = 2675;
  int num_testing_examples = 2000;
  int num_columns = num_observed_nodes + 1;
  int num_obs_vars = num_observed_nodes;

  string train_file_name = "language_train" + to_string(trial) + ".txt";
  string test_file_name = "language_test" + to_string(trial) + ".txt";
  ...
  while (getline(test_file,test_line) && curr_test_index < num_testing_examples)
  {
	  cout << curr_test_index;
	  cout << "\n";
	  ...
	  for (int i = 0; i < num_obs_vars; ++i)
	  {
		...
		test_samples->Set(curr_test_index, num_nodes - 1 - i, val);
	  }
	  curr_test_index++;
  }
  test_file.close();
  ...
  while (getline(train_file,line) && curr_line < num_training_examples)
  {
	  ...
	  for (int i = 0; i < num_obs_vars; ++i)
	  {
		...
		if (class_var == 0)
		{
			train_samples0->Set(curr_row0, num_nodes - 1 - i, val);
		}
		if (class_var == 1)
		{
			train_samples1->Set(curr_row1, num_nodes - 1 - i, val);
		}
	  }
	  if (class_var == 0)
		  curr_row0++;
 ...
 /* int EM_jtree_nodes = num_observed_nodes;
  ...
  /* learn spectral parameters *****************************************************************/
   TensorJTree jtree0(bayesNet0, tree_matrix, nodes_to_vars, stack_count, jtemplate_vec, max_runs);
	 ...
	 double spectral_time0 = 0;
	 double spectral_time1 = 0;
  if (alg_flag == 0)
  {
  ...
  spectral_time0 = ((double)spectral_final0) / ((double) CLOCKS_PER_SEC);

  clock_t spectral_init1 = clock();

  jtree1.LearnSpectralParameters();
  clock_t spectral_final1 = clock() - spectral_init1;
  spectral_time1 = ((double)spectral_final1) / ((double) CLOCKS_PER_SEC);
  }
  /*******************************************************************************************/
...
//	double prob = TensorVarElim::VE(*bayesNet, evidence_vars, evidence_vals, bayesNet->ReverseTopologicalOrder());
	...
	/*	cout << spectral_val2;
		...
		if (spectral_val0 >= spectral_val1)
		{
			spectral_map = 0;
		}
		else if (spectral_val1 > spectral_val0)
		{
			spectral_map = 1;
		}
		else
	...
	if (alg_flag == 2)
	{
		...
		if (EM_online_val0 >= EM_online_val1)
		{
			online_EM_map = 0;
		}
		else if (EM_online_val1 > EM_online_val0)
		{
			online_EM_map = 1;
		}
		else
...
/*	cout << EM_val0;
	...
	if (spectral_map != test_labels[n])
	{
		spectral_error++;
	}
	if (EM_map != test_labels[n])
	{
		EM_error++;
	}
	if (online_EM_map != test_labels[n])
	{
		online_EM_error++;
	}
  }
  ...
  cout << "yeeee2\n";
  double spectral_time = spectral_time0 + spectral_time1;
  cout << "yeeee3\n";
  double EM_time =  EM_time0 + EM_time1;
  double onlineEM_high_time = onlineEM_high_time0 + onlineEM_high_time1;
  cout << "yeeee4\n";
  cout << "total error:\n";
  cout << spectral_error;
  cout << "\n";
  ...
  string outfile_name = "language-reverse-" + to_string(num_hidden_states) + "-"
						+ to_string(num_training_examples)
						+ "-" + to_string(EM_thresh) 
						+ "-" + to_string(stack_count)
						+ "-" + to_string(max_runs)
						+ "-" + to_string(trial) + "-" + to_string(alg_flag) + ".txt";
//  string outfile_name = "fun.txt";
  ...
  myfile << ("EM\t" + to_string(EM_error) + "\t" + to_string(EM_time) + "\t" + to_string(curr_top_online_high_like1) + "\n");
  myfile << ("online EM\t" + to_string(online_EM_error) + "\t" + to_string(onlineEM_high_time) + to_string(curr_top_online_high_like1) + "\n");
