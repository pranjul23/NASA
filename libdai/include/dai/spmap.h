...
public:
	...
	spvector_map() : _p(), _size(0), _def((T)0) {}

	explicit spvector_map( size_t n, T p ) : _size(n), _def((T)0){
		if(p != (T)0){
	...
	spvector_map( TIterator begin, TIterator end, size_t sizeHint) : _p(), _size(0), _def((T)0) {
		if( sizeHint )
			reserve( sizeHint );
		size_t iter = 0;
	...
	template <typename S>
	spvector_map( const std::vector<S> &v, size_t sizeHint) : _p(), _size(v.size()), _def((T)0) {
		if( sizeHint )
			reserve( sizeHint );
