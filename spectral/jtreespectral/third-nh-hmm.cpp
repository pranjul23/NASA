...
/*  int num_hidden_nodes = 20;
  ...
  for (int i = 0; i < num_hidden_nodes; ++i)
  {
	  ...
	  if (i >= 3)
	  {
		  parent_list[hidden_nodes[i]]->push_back(hidden_nodes[i-3]);
		  template_vec[hidden_nodes[i]] = i;
	  }
  }
...
/*  int jtree_nodes = num_observed_nodes - skip;
  ...
  for (int i = 0; i < jtree_nodes;  ++i)
  {
	  ...
	  if (i + 3 < jtree_nodes)
		  nodes_to_vars[i]->push_back(hidden_nodes[i+3 + skip]);
  } */
...
/* vector<int> jtemplate_vec(jtree_nodes, 0);
 ...
 /* int EM_jtree_nodes = num_observed_nodes;
  ...
  JTree oracletree(bayesNet, EM_tree_matrix, EM_nodes_to_vars, train_samples, EM_thresh);
  oracletree.LearnEMParameters(0, 1);
  ...
  JTree EMtree(bayesNet, EM_tree_matrix, EM_nodes_to_vars, train_samples, EM_thresh);
  double like1 = EMtree.LearnEMParameters(1, 0);
  clock_t EM1_final = clock() - EM1_init;
  double EM1_time = ((double)EM1_final) / ((double) CLOCKS_PER_SEC);

  clock_t EM2_init = clock();
  JTree EMtree2(bayesNet, EM_tree_matrix, EM_nodes_to_vars, train_samples, EM_thresh);
  double like2 = EMtree2.LearnEMParameters(50, 0);
  clock_t EM2_final = clock() - EM2_init;
  double EM2_time = ((double)EM2_final) / ((double) CLOCKS_PER_SEC);

  clock_t EM3_init = clock();
  JTree EMtree3(bayesNet, EM_tree_matrix, EM_nodes_to_vars, train_samples, EM_thresh);
  double like3 = EMtree3.LearnEMParameters(10, 0);
  clock_t EM3_final = clock() - EM3_init;
  double EM3_time = ((double)EM3_final) / ((double) CLOCKS_PER_SEC);

  clock_t EM4_init = clock();
  JTree EMtree4(bayesNet, EM_tree_matrix, EM_nodes_to_vars, train_samples, EM_thresh);
  double like4 = EMtree4.LearnEMParameters(50, 0);
  clock_t EM4_final = clock() - EM4_init;
  double EM4_time = ((double)EM4_final) / ((double) CLOCKS_PER_SEC);

  clock_t EM5_init = clock();
  JTree EMtree5(bayesNet, EM_tree_matrix, EM_nodes_to_vars, train_samples, EM_thresh);
  double like5 = EMtree5.LearnEMParameters(10, 0);
...
//	double prob = TensorVarElim::VE(*bayesNet, evidence_vars, evidence_vals, bayesNet->ReverseTopologicalOrder());
	 double prob = oracletree.ComputeEmpiricalMarginals(marginals, evidence_vars, evidence_vals);
...
//	cout << "\n";
  ...
  string outfile_name = "third-nh-hmm-" + to_string(num_hidden_nodes) + "-" + to_string(num_hidden_states) + "-" 
