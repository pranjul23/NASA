...
int main (int argc, char *argv[])
{
 ...
 int num_hidden_nodes = 12;
  int num_hidden_states = 2;
  int num_observed_nodes = 12;
  ...
  for (int i = 0; i < num_hidden_nodes; ++i)
  {
	  ...
	  if (i >= 2)
	  {
		  ...
		  if ( i < num_hidden_nodes / 2)
			template_vec[hidden_nodes[i]] = 2;
		  else
			  template_vec[hidden_nodes[i]] = 3;
	  }
  }
...
//  vector<int> jtemplate_vec;
 // VectorPlus::Seq(jtemplate_vec, 0, 1, jtree_nodes);
  ...
  {
	  if (hidden_nodes[i + skip + 2] < num_hidden_nodes / 2)
		jtemplate_vec[i] = 1;
	  else
		  jtemplate_vec[i] = 2;
  }
  jtemplate_vec[jtree_nodes/2 - 2] = 3;
  jtemplate_vec[jtree_nodes/2 - 1] = 4;

  for (int i = jtree_nodes/2; i < jtree_nodes; ++i)
  {
	  if (i - jtree_nodes / 2 + skip < num_observed_nodes / 2)
		jtemplate_vec[i] =  5;
	  else
		  jtemplate_vec[i] =  6;
  }
 ...
 /* int EM_jtree_nodes = num_observed_nodes;
  ...
  jtree.LearnTemplateSpectralParameters();
  clock_t spectral_final = clock() - spectral_init;
  double spectral_time = ((double)spectral_final) / ((double) CLOCKS_PER_SEC);

    TensorJTree jtree2(bayesNet, tree_matrix, nodes_to_vars, stack_count, jtemplate_vec, max_runs);
