...
% trains spectral algoritm, returns "transform_array" which is cell array
% of observable parameters

function transform_array = SpectralTrainSystem(tree_matrix, type_indicator, Ko, Kh, training_samples, system_size)

    total_vars = size(tree_matrix, 1);
    
    true_CPT = cell(total_vars, 1); % this is just a dummy array that used to be used for debugging
    
    EmpiricalProbMap = java.util.HashMap;
	% compute transformed version of each variable
	transform_array = cell(total_vars, 1); % cell array that holds "transformed parameters"
    
	total_vars = size(tree_matrix, 1);
	global_root = find_root(tree_matrix);
		
    best_condition_matrix = ComputeConditionNumbers(training_samples, type_indicator, Ko, Kh, EmpiricalProbMap);
	...
	for v=1:1:total_vars
		...
		if (type_indicator(v) == 0)
			...
			[transform_array, EmpiricalProbMap] = compute_transform_obs(v, transform_array, training_samples, ...
																	   type_indicator, Ko, Kh, tree_matrix, true_CPT, EmpiricalProbMap);
...
function  [transform_array, EmpiricalProbMap] = compute_transform_hidden(node_id, transform_array, training_samples, ...
                                                        type_indicator, Ko, Kh, tree_matrix, true_CPT, EmpiricalProbMap)
...
function [transform_array, EmpiricalProbMap] = compute_transform_obs(node_id, transform_array, training_samples, ...
                                                                        type_indicator, Ko, Kh, tree_matrix, CPT, EmpiricalProbMap)
...
% conditioned one
function extra_obs = find_extra_obs(first_obs, invalid_ancestors, best_condition_matrix, full_tree_matrix, ...
                                    type_indicator, tree_matrix, system_size)
