...
void TestHMM::test_marginal(const char* filename, size_t ID){
	...
	for(size_t i=0; i<test_data.size(); i++) {
		...
		//initialize HMM of ever increasing size up to test_data[i].size()
		graph = new FactorGraph();
		graph->createHMMFactorGraph(param.init, param.dist, test_data[i].size());
		...
		O_last = jt->calcMarginal(graph->var(2));
	...
	ofstream os;
	stringstream result;
	result << string("data/HMMmarginal_test_") << ID << string(".txt");
	os.open(result.str().c_str(), ios::trunc);
...
void TestHMM::test_marginal_cut(const char* filename, size_t ID){
